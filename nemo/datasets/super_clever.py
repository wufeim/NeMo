...
class SuperClever(Dataset):
    def __init__(
        ...
        **kwargs,
        ...
        self.file_list = ['superCLEVR_new_%06d' % i for i in [118]]

        self.image_path = os.path.join(self.root_path, "images_new")
        self.annotation_path = os.path.join(self.root_path, "scenes_new")
        self.mask_path = os.path.join(self.root_path, "masks_new")
...
if __name__ == '__main__':
    ...
    mesh_path = '/mnt/sdb/data/cs/output/save_models_2'
    mesh_sub_cates = [t.split('.')[0] for t in os.listdir(mesh_path)]
    ...
    import tqdm
    for image_idx in tqdm.trange(100, 120):

        anno_ = json.load(open("/mnt/sdb/data/cs/output/scenes_new/superCLEVR_new_%06d.json" % image_idx))
        # anno_['objects'] = anno_['objects'][1::]
        img_ = np.array(Image.open('/mnt/sdb/data/cs/output/images_new/superCLEVR_new_%06d.png' % image_idx))
        ...
        Image.fromarray((img[..., 3] * 255).astype(np.uint8)).save('/mnt/sdb/data/cs/output/masks_new/superCLEVR_new_%06d.png' % image_idx)
